...
import (
	...
	"github.com/districtdco/accessd/api/internal/audit"
	...
	"github.com/districtdco/accessd/api/internal/sshproxy"
...
func New(_ context.Context, cfg config.Config, logger *slog.Logger, pool *pgxpool.Pool) (*App, error) {
	...
	sshProxyServer, err := sshproxy.New(sshproxy.Config{
		...
		UpstreamHostKeyAutoFix: cfg.SSHProxy.UpstreamAutoRepair,
...
func (a *App) bootstrapDevAccess(ctx context.Context) error {
	...
	seedAssets := []struct {
	...
	}{
		{
			...
			username:  "ubuntu",
			secret:    "accessd-dev-linux-app-01",
		...
		},
		{
			...
			username:  "ec2-user",
			secret:    "accessd-dev-linux-batch-01",
			credType:  credentials.TypePassword,
			actions:   []string{access.ActionShell, access.ActionSFTP},
		},
		{
			...
			username:  "opsadmin",
			secret:    "accessd-dev-linux-ops-01",
			credType:  credentials.TypePassword,
			actions:   []string{access.ActionShell, access.ActionSFTP},
		},
		{
			...
			username:  "app_user",
			secret:    "accessd-dev-db-password",
		...
		},
		{
			...
			username:  "app_user",
			secret:    "accessd-dev-mysql-password",
			credType:  credentials.TypeDBPassword,
			actions:   []string{access.ActionDBeaver},
		},
		{
		...
		},
		{
			...
			username:  "default",
			secret:    "accessd-dev-redis-password",
			credType:  credentials.TypePassword,
