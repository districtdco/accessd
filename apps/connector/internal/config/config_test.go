...
func TestDeriveDefaultBackendVerifyURL_UsesFirstValidOrigin(t *testing.T) {
	got := deriveDefaultBackendVerifyURL([]string{
		...
		"https://accessd.districtd.lan",
	})
	want := "https://accessd.districtd.lan/api/connector/token/verify"
	if got != want {
...
func TestDeriveOriginFromURL(t *testing.T) {
	got := deriveOriginFromURL("https://accessd.districtd.lan/api/connector/token/verify")
	want := "https://accessd.districtd.lan"
	if got != want {
...
func TestAppendUniqueOrigin_DeduplicatesCaseInsensitive(t *testing.T) {
	got := appendUniqueOrigin([]string{"https://accessd.districtd.lan"}, "HTTPS://ACCESSD.DISTRICTD.LAN")
	if len(got) != 1 {
...
func TestDeriveDefaultBackendCACertFile_FindsHostCert(t *testing.T) {
	...
	want := filepath.Join(certsDir, "accessd-accessd.districtd.lan.cer")
	if err := os.WriteFile(want, []byte("test-cert"), 0o644); err != nil {
		t.Fatalf("write cert file: %v", err)
	}

	got := deriveDefaultBackendCACertFile("https://accessd.districtd.lan/api/connector/token/verify")
	if got != want {
